(function() {
    ...
    exports.fieldextract = async function(obj, callback) {
        var boundaryKey = randString(30);

        let rootPath = process.mainModule.paths[0].split('node_modules')[0].slice(0, -1);

        let filename = "";

        if(obj.filepath) {
            filename =  getFileName(obj.filepath)
            filename = rootPath + '/assets/temp/' + filename;
        }

        if(obj.fileUrl) {
            console.log("There is a url defined, downloading file");

            console.log('rootPatth', rootPath);
            let fullpath = rootPath + '/assets/temp/' + obj.fileName;
            let downloadFile = null;
            try {
                downloadFile = await downloader.download(obj.fileUrl, fullpath, 0)
            ...
            if(downloadFile.success) {
                if(downloadFile.success === true){
                    filename = fullpath;
                }else {
                    console.log('downloadFile', downloadFile);
                    return callback('Error downloading file');
                }
        ...
        console.log('did the await work?', filename);
        ...
        var reader = fs.createReadStream(filename, { bufferSize: 64 * 1024 });
        var closingLine = '\r\n------FormBoundary' + boundaryKey + '--';
        ...
        console.log('options', common.options);
    ...
    exports.list = function(obj, callback) {
        ...
        common.options.headers['Content-Length'] = 0;
